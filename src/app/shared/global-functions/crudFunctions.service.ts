...
export class CRUDFunction {
  ...
  constructor(private toastrService: ToastrService, private NgbModal: NgbModal, private router: Router, private activatedRoute: ActivatedRoute) { }
  ...
  apiResponce(res, responceMessage) {
    ...
    if (res.status) {
      success = true;
      this.toastrService.success(responceMessage+' '+ res.message + (responceMessage ? ' Successfully' : ''));
