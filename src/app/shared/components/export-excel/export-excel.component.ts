...
export class ExportExcelComponent implements OnInit {
  ...
  constructor(private dialogRef: MatDialogRef<ExportExcelComponent>, @Inject(MAT_DIALOG_DATA) public data: any, private CRUDFunction: CRUDFunction, private formateDateService: FormateDateService, private toastrService: ToastrService) { }
  ...
  excelExport(check) {
    if (this.customExport.length != 0) {
      ...
      console.log(value, key, this.data);
      this.data.visibleTableList.forEach((x, i) => {
        let post = {};
        key.forEach((element, i) => {
          key.includes(...key) ? post[key[i]] = (isDate[i] && x[value[i]]) ? this.formateDateService.formatDate(x[value[i]]) : x[value[i]] : '';
        });
