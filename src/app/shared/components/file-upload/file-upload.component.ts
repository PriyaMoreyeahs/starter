...
export class FileUploadComponent implements ControlValueAccessor {
  ...
  constructor(private host: ElementRef<HTMLInputElement> , private toast: ToastrService) {}
  ...
  fileSelect(event) {
    ...
    if (event[0].type.match("application/*")){ 
      ...
      this.filename = event[0].name;
    }else if(!event[0].type.match("image.*") && !event[0].type.match("application/*") && !event[0].type.match("application/*")){
      this.toast.error("Invalid file type. Please upload in proper formate.");
    }
