...
export class SigninComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.loginForm = this.formBuilder.group({
      UserName: ["", Validators.compose([Validators.required, Validators.pattern("^[A-Za-z0-9._%+-]+@[a-z0-9.-]+.[a-z]{2,4}$"),]),],
      Password: ["", Validators.required],
    });

    this.url1 = location.origin;
    this.url = environment.config.API_URL
    ...
    this.authService.getuserlogindetail(data).subscribe((data: any) => {
      if (data != null) {
        ...
        if (this.profiledata.roleType == "SuperAdmin") {
          this.router.navigate(["/super-admin/superadmin-dasboard"]).then(() => {
          });
          localStorage.setItem(btoa("ROLE"), btoa(JSON.stringify("SuperAdmin")));
        }
        else {
  ...
  onSubmit(value: boolean) {
    ...
    if (this.loginForm.invalid) {
      this.login = false
      return this.ToastrService.error("Please Use Proper Login Credentials For Login");
    }
    let post = {
