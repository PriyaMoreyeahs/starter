...
export class AddUpdateAssetsComponent implements OnInit {
  ...
  constructor(private CRUDFunction: CRUDFunction, private assetsService: AssetsService, private formBuilder: FormBuilder, private formateDateService: FormateDateService, private toastrService: ToastrService) { }
  ...
  baseCategorySelection(baseCategoryId) {
    this.assetsService.getallcategorybybasecategoryid(baseCategoryId).subscribe((res: any) => {
  ...
  patchValue(assetInformation) {
    ...
    this.addUpdateAssetsForm.patchValue({
      ...
      assignToId: (assetInformation.assetCondition == 3 || assetInformation.assetCondition == 4) ? '' : assetInformation.assignToId,
    });
  }
  //#endregion
  ...
  conditionSelection(conditionId) {
    ...
    (conditionId == 3 || conditionId == 4) ? (this.assignable = false, this.addUpdateAssetsForm.get('assignToId').setValue('')) : this.assignable = true;
  }
  //#endregion
  ...
  submitValue(value) {
    ...
    value['warrantyExpDate'] = value['warrantyExpDate'] ? this.formateDateService.formatDate(value['warrantyExpDate']) : value['warrantyExpDate'];
    value = { ...value, ...images };
    let apiName = '';
    !this.assetsId ? apiName = 'additemmaster' : (apiName = 'edititemmaster', value['itemId'] = this.assetsId)
    this.assetsService[apiName](value).subscribe((res: any) => {
      this.CRUDFunction.apiResponce(res, '') ? this.CRUDFunction.routingWithOutData('assets-master/company-assets') : '';
    }, (error) => {
      this.CRUDFunction.handleHttpError(error);
    })
  }
