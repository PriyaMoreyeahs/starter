...
export class AssetsWarehouseAndCategoryComponent implements OnInit {
  ...
  searchString: string = '';
  warehouseList: any;
  configWarehouseList: { itemsPerPage: number; currentPage: number; totalItems: string; };
  warehouseId: any;
  addUpdateWarehouseForm: FormGroup;
  submitted: boolean = false;
  activePageId: any = 1;
  configBaseCategoryList: { itemsPerPage: number; currentPage: number; totalItems: string; };
  configCategoryList: { itemsPerPage: number; currentPage: number; totalItems: string; };
  ...
  filesArray: any = [];
  api: any = environment.config.API_URL;

  constructor(private CRUDFunction: CRUDFunction, private assetsService: AssetsService, private popupFunctionService: PopupFunctionService, private formBuilder: FormBuilder, private toastrService: ToastrService) {
  ...
  searchInput(value) {
    if (value.length > 2) {
      this.searchStringWarehouse = value;
    } else {
      this.searchStringWarehouse = '';
    }
  }
  //#endregion
  ...
  getallwarehouse() {
    ...
    this.assetsService.getallwarehouse(post).subscribe((res: any) => {
  ...
  submitValue(value) {
    let apiName = '';
    !this.warehouseId ? (apiName = 'addwarehouse') : (apiName = 'updatewarehouse', value['warehouseId'] = this.warehouseId);
  ...
  deleteWarehouse(warehouseId) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.assetsService.deletewarehouse(warehouseId).subscribe((data: any) => {
          if (data.status) {
            Swal.fire(
              "Deleted!",
  ...
  addUpdateBaseCategoryPopUp(addUpdateBaseCategoryComponent, baseCategoryId?) {
    (this.baseCategoryId = baseCategoryId) ? this.getbasecategorybyid(baseCategoryId) : '';
    this.getassetstypeenum();
    this.addUpdateBaseCategoryForms();
    this.popupFunctionService.poupOpenFunction(addUpdateBaseCategoryComponent, 'lg');
  }
  //#endregion
  ...
  addUpdateCategoryPopUp(addUpdateCategoryComponent, categoryId?) {
    ...
    this.popupFunctionService.poupOpenFunction(addUpdateCategoryComponent, 'lg');
  }
  //#endregion
  ...
  getallbasecategory() {
    ...
    this.assetsService.getallbasecategory(post).subscribe((res: any) => {
      let post = this.CRUDFunction.responceBindingInPagination(res);
      this.baseCategoryList = post['tableData'];
      this.configBaseCategoryList.totalItems = post['totalLength'];    }, (error) => {
      this.CRUDFunction.handleHttpError(error);
    });
  }
  //#endregion
  ...
  getallcategory() {
    ...
    this.assetsService.getallcategory(post).subscribe((res: any) => {
      let post = this.CRUDFunction.responceBindingInPagination(res);
  ...
  addUpdateBaseCategory(value) {
    this.submitted = true;
    this.CRUDFunction.checkValidation(value) ? this.submitBaseCategory(value.value) : '';
  }
  submitBaseCategory(value) {
    let apiName = '';
    !this.baseCategoryId ? apiName = 'addbasecategory' : (apiName = 'updatebasecategory', value['assetsBCategoryId'] = this.baseCategoryId);
    this.assetsService[apiName](value).subscribe((res: any) => {
      this.CRUDFunction.apiResponce(res, '') ? (this.getallbasecategory(), this.paginator.firstPage()) : '';
    }, (error) => {
      this.CRUDFunction.handleHttpError(error);
    });
  }
  ...
  addUpdateCategory(addUpdateCategoryForm) {
    this.submitted = true;
    this.CRUDFunction.checkValidation(addUpdateCategoryForm) ? this.submitCategory(addUpdateCategoryForm.value) : '';
  }
  submitCategory(value) {
    ...
    let apiName = '';
    !this.categoryId ? apiName = 'addcategory' : (apiName = 'updatecategory', post['assetsCategoryId'] = this.categoryId);
    this.assetsService[apiName](post).subscribe((res: any) => {
      this.CRUDFunction.apiResponce(res, '') ? (this.getallcategory(), this.paginator.firstPage()) : '';
    }, (error) => {
      this.CRUDFunction.handleHttpError(error);
    });
  }
  ...
  uploadFiles(files: FileList) {
    ...
    files.length <= 1 ? (!this.CRUDFunction.checkFileSizeAndType(files, 'image') ? this.upload(files) : '') : '';
  }
  ...
  upload(files) {
    for (let index = 0; index < files.length; index++) {
      ...
      this.assetsService.uploadeassetsiconimg(formData).subscribe((res: any) => {
        if (res.status) {
          let responce = res.data.path;
          this.filesArray.length < 1 ? this.filesArray.push(responce) : this.toastrService.error('Only One file Can be uploaded');
  ...
  getbasecategorybyid(baseCategoryId) {
    this.assetsService.getbasecategorybyid(baseCategoryId).subscribe((res: any) => {
      let data = this.CRUDFunction.responceBindingAsObject(res);
  ...
  getcategorybyid(categoryId) {
    this.assetsService.getcategorybyid(categoryId).subscribe((res: any) => {
      let data = this.CRUDFunction.responceBindingAsObject(res);
