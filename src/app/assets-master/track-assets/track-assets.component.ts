...
export class TrackAssetsComponent implements OnInit {
  ...
  constructor(private assetsService: AssetsService, private CRUDFunction: CRUDFunction, private popupFunctionService: PopupFunctionService, private formBuilder: FormBuilder, private toastrService: ToastrService) {
    this.config = {
      itemsPerPage: 10,
      currentPage: 1,
      totalItems: '',
  ...
  getallassigneassest() {
    ...
    this.assetsService.getallassigneassest(post).subscribe((res: any) => {
  ...
  upload(files) {
    for (let index = 0; index < files.length; index++) {
      ...
      this.assetsService.recoverassetsimages(formData).subscribe((res: any) => {
        if (res.success) {
          let responce = res.pathArray[0];
          this.filesArray.length < 5 ? this.filesArray.push(responce) : this.toastrService.error('Only 5 file can be uploaded');        } else {
  ...
  recoverAssetsFromEmployee(value) {
    this.submitted = true;
    this.CRUDFunction.checkValidation(value) ? this.recoverAssets(value.value) : '';
  }
  recoverAssets(value) {
    let post = {
      itemId: this.itemId,
    ...
    this.assetsService.recoverdassets(post).subscribe((res: any) => {
      this.CRUDFunction.apiResponce(res, '') ? this.getallassigneassest() : '';
    }, (error) => {
      this.CRUDFunction.handleHttpError(error);
    });
  }
