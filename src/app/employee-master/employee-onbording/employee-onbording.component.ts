...
export class EmployeeOnbordingComponent implements OnInit {
  ...
  onbordingInformation: any;  constructor(private CRUDFunction: CRUDFunction, private employeeService: EmployeeService) {
    this.config = {
      itemsPerPage: 10,
      currentPage: 1,
      totalItems: '',
  ...
  getnewemployeeonboarding() {
    ...
    this.employeeService.getnewemployeeonboarding(post).subscribe((res: any) => {
      this.onbordingInformation = this.CRUDFunction.responceBindingInPagination(res);
