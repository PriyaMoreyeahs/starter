...
export class AddUpdateEmployeeComponent implements OnInit {
  ...
  bloodGroupList: any[];
  maritalStatus: any = ['Not Set', 'Unmarried', 'Married', 'Divorced', 'Widow', 'Widower']
  ...
  constructor(private CRUDFunction: CRUDFunction, private formBuilder: FormBuilder, private employeeService: EmployeeService, private organizationService: OrganizationService, private formateDateService: FormateDateService) { }
  ...
  dataFromURL() {
    ...
    this.information?.message == 'Add' ? this.initialInformationForms() : this.information?.message == 'Add Candidate' ? this.addCandidate(this.information?.employeeId) : this.getnewemployeebyid(this.information?.employeeId);
  ...
  getnewemployeebyid(employeeId) {
    let apiName = ''
    this.information?.message == 'Edit' ? apiName = 'getnewemployeebyid' : apiName = 'getnewemployeeonboardingbyid';
    this.employeeService[apiName](employeeId).subscribe((res: any) => {

      let employeeInfo = this.CRUDFunction.responceBinding(res);
      employeeInfo ? (this.patchInitialInformations(employeeInfo), this.patchPersonalInformation(employeeInfo), this.patchBankInformation(employeeInfo), this.patchOfficalInformation(employeeInfo)) : '';
    }, (error) => {
  ...
  patchOfficalInformation(employeeInfo) {    this.departmentSelectioin(employeeInfo.departmentId);
    this.officalInformationForm.patchValue({
      employeeTypeId: employeeInfo.employeeTypeId,
      departmentId: employeeInfo.departmentId == 0 ? null : employeeInfo.departmentId,
      designationId: employeeInfo.designationId == 0 ? null : employeeInfo.designationId,
      ...
      reportingManagerId: employeeInfo.reportingManagerId == 0 ? null : employeeInfo.reportingManagerId,
  ...
  getemployeetype() {
    this.employeeService.getemployeetype().subscribe((res: any) => {
  ...
  getallactivedeparmentlist() {
    this.organizationService.getallactivedeparmentlist().subscribe((res: any) => {
  ...
  departmentSelectioin(departmentId) {
    this.organizationService.designationbydepartment(departmentId).subscribe((res: any) => {
  ...
  emplistnotself(information) {
    let apiName = '';
    information?.message == 'Edit' ? apiName = 'emplistnotself' : apiName = 'getallemployeelist';
    this.employeeService[apiName](information?.employeeId).subscribe((res: any) => {
  ...
  getshiftgroup() {
    this.employeeService.getshiftgroup().subscribe((res: any) => {
      this.shiftGroupList = this.CRUDFunction.responceBinding(res);    }, (error) => {
      this.CRUDFunction.handleHttpError(error);
    });
  }
  //#endregion
  ...
  getallweekoffsgroup() {
    this.employeeService.getallweekoffsgroup().subscribe((res: any) => {
  ...
  getbloodtype() {
    this.employeeService.getbloodtype().subscribe((res: any) => {
  ...
  orgbycompanyidclaims() {
    this.employeeService.orgbycompanyidclaims().subscribe((res: any) => {
  ...
  submitEmployeeInformations(initialInformationForm) {
    this.submitted = true;
    this.CRUDFunction.checkValidation(initialInformationForm) ? this.submitInitialValue(initialInformationForm.value) : '';
  }

  submitInitialValue(value) {
    value['dateOfBirth'] = this.formateDateService.formatDate(value['dateOfBirth']);
    let post = {
      onBoardingStage: 1,
    ...
    this.information?.message == 'Add' ? (apiName = 'createemployeeentity', returnUrl = 'employee-master/employee-onboarding', post = value)
      : this.information?.message == 'Edit' ? (apiName = 'updatenewemployee', post['employeeId'] = this.information?.employeeId, returnUrl = 'employee-master/all-employee')
        : (apiName = 'updatenewemployeeonboarding', post['employeeOnBoardingId'] = this.information?.employeeId, returnUrl = 'employee-master/employee-onboarding');

    this.employeeService[apiName](post).subscribe((res: any) => {
      this.CRUDFunction.apiResponce(res, 'Employee Onbording') ? this.CRUDFunction.routingWithOutData(returnUrl) : '';
    }, (error) => {
      this.CRUDFunction.handleHttpError(error);
    });
  }
  ...
  submitPersoanlInformation(personalInformationForm) {
    this.submitted = true;
    this.CRUDFunction.checkValidation(personalInformationForm) ? this.submitPersonalValue(personalInformationForm.value) : '';
  }

  submitPersonalValue(value) {

    value['dobPerAdhaar'] = this.formateDateService.formatDate(value['dobPerAdhaar']);
    value['passportValidUntil'] = this.formateDateService.formatDate(value['passportValidUntil']);
    let apiName = '';
    let post = {
      onBoardingStage: 2,
      updatePersonalInformations: value
    }
    this.information?.message == 'Edit' ? (apiName = 'updatenewemployee', post['employeeId'] = this.information?.employeeId) : (apiName = 'updatenewemployeeonboarding', post['employeeOnBoardingId'] = this.information?.employeeId);
    this.employeeService[apiName](post).subscribe((res: any) => {
  ...
  submitBankInformation(bankInformationForm) {
    this.submitted = true;
    this.CRUDFunction.checkValidation(bankInformationForm) ? this.submitBankValue(bankInformationForm.value) : '';
  }
  ...
  submitOfficialInformatrion(officalInformationForm) {
    this.submitted = true;
    this.CRUDFunction.checkValidation(officalInformationForm) ? this.submitOfficialValue(officalInformationForm.value) : '';
  }

  submitOfficialValue(value) {

    value['joiningDate'] = this.formateDateService.formatDate(value['joiningDate']);
    value['confirmationDate'] = this.formateDateService.formatDate(value['confirmationDate']);
    let post = {
      onBoardingStage: 4,
    ...
    let apiName = ''
    let returnURL = ''
    this.information?.message == 'Edit' ? (apiName = 'updatenewemployee', post['employeeId'] = this.information?.employeeId, returnURL = 'employee-master/all-employee') : (apiName = 'updatenewemployeeonboarding', post['employeeOnBoardingId'] = this.information?.employeeId, returnURL = 'employee-master/employee-onboarding');
    this.employeeService[apiName](post).subscribe((res: any) => {
      this.CRUDFunction.apiResponce(res, 'Official Information') ? this.CRUDFunction.routingWithOutData(returnURL) : '';
    }, (error) => {
      this.CRUDFunction.handleHttpError(error);
    })
  }
