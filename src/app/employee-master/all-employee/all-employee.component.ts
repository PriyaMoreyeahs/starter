...
export class AllEmployeeComponent implements OnInit {
  ...
  constructor(private employeeService: EmployeeService, private CRUDFunction: CRUDFunction, private organizationService: OrganizationService) {
    this.config = {
      itemsPerPage: 10,
      currentPage: 1,
    };
  ...
  empdirectoryfilter() {
    ...
    this.employeeService.empdirectoryfilter(post).subscribe((res: any) => {
      this.employeeInformation = this.CRUDFunction.responceBindingAsObject(res);
    }, (error) => {
  ...
  employeeActivationChange(employeeId) {
    this.employeeService.lockemployee(employeeId).subscribe((res: any) => {
  ...
  getallactiveorg() {
    let seach = ''
    this.organizationService.getallactiveorg(seach).subscribe((res: any) => {
  ...
  addUpdateEmployee(employeeId?) {
    ...
    this.CRUDFunction.routingWithData('employee-master/add-update-employee', post);
  }
  //#endregion
}
