...
export class AddUpdateTicketComponent implements OnInit {
  ...
  employeeList: any[];
  priorityList: any = [
    { priorityType: 1, name: 'Low', isRequired: false, icon: 'star_border', color: '#ff9800' },
    { priorityType: 2, name: 'Medium', isRequired: false, icon: 'star_half', color: '#2196f3' },
    { priorityType: 3, name: 'High', isRequired: false, icon: 'star', color: '#ec8686' },
    { priorityType: 4, name: 'Urgent', isRequired: false, icon: 'warning', color: '#f44336' },
    { priorityType: 5, name: 'Road Blocker', isRequired: false, icon: 'whatshot', color: '#c2b89d' },
  ];
  ...
  constructor(private CRUDFunction: CRUDFunction, private ticketService: TicketService, private formBuilder: FormBuilder, private toastrService: ToastrService) { }
  ...
  addUpdateCategoryForms() {
    this.addUpdateCategoryForm = this.formBuilder.group({
      ...
      categoriesAssign: ['', Validators.required],
  ...
  emplistoforg() {
    this.ticketService.emplistoforg().subscribe((res: any) => {
  ...
  submitCategory(value) {
    this.submitted = true;
    this.CRUDFunction.checkValidation(value) ? this.checkFormArray(value.value) : '';
  }

  checkFormArray(value) {
    let errorBox = []
    value.subCategories.forEach(element => {
      !element.subCategoriesName ? errorBox.push(true) : errorBox.push(false);
    });
    errorBox.some(element => element === true) ? this.toastrService.error('Please enter sub category name') : this.submitValue(value);
  }
  //#endregion
  ...
  submitValue(value) {
    ...
    let apiName = '';
    this.categoryId ? (apiName = 'updatecategory', value['categoryId'] = this.categoryId) : apiName = 'createcategory';
    this.ticketService[apiName](value).subscribe((res: any) => {
      this.CRUDFunction.apiResponce(res, 'Ticket Category') ? this.CRUDFunction.routingWithOutData('/help-desk/ticket-category') : '';
    }, (error: any) => {
      this.CRUDFunction.handleHttpError(error);
    });
  }
  ...
  getticketcategorybyid(categoryId) {
    this.ticketService.getticketcategorybyid(categoryId).subscribe((res: any) => {
  ...
  patchDataInTheCategoryForm(ticketCategoryData) {
    ...
    ticketCategoryData.subCategories.length != 0 ? this.selected = '1' : this.selected = '2';
    this.selected == '1' ? this.setsubcategories(ticketCategoryData.subCategories) : '';
  }
  //#endregion
