...
export class TicketReportComponent implements OnInit {
  ...
  constructor(private ticketService: TicketService, private CRUDFunction: CRUDFunction, private formateDateService: FormateDateService) {
    this.config = {
      itemsPerPage: 10,
      currentPage: 1,
      totalItems: '',
  ...
  reportSearch(check: boolean) {
    let post = {
     ...
     this.ticketService.getticketdata(post).subscribe((res: any) => {
      if (check) {
        ...
        this.CRUDFunction.exportAsExcelFile(post['tableData'], "TiecktReport" + new Date());
      }
    }, (err: any) => {
