...
export class TicketChatComponent implements OnInit {
  priorityIcons = [
    { class: 'low-icon', iconName: 'star_border', name: 'Low', color: '#ff9800', priorityType: 1 },
    { class: 'medium-icon', iconName: 'star_half', name: 'Medium', color: '#2196F3', priorityType: 2 },
    { class: 'hign-icon', iconName: 'star', name: 'High', color: '#ec8686', priorityType: 3 },
    { class: 'urgent-icon', iconName: 'warning', name: 'Urgent', color: '#f44336', priorityType: 4 },
    { class: 'roadblock-icon', iconName: 'whatshot', name: 'Road Blocker', color: '#c2b89d', priorityType: 5 },
  ];
  status = [
    { id: 1, name: "Pending" },
    { id: 2, name: "Progress" },
  ...
  constructor(private CRUDFunction: CRUDFunction, private ticketService: TicketService, private popupFunctionService: PopupFunctionService, private toastrService: ToastrService, private formBuilder: FormBuilder) { }
  ...
  getticketdetails() {
    this.ticketService.getticketdetails(this.ticketId).subscribe((res: any) => {
      ...
      this.ticketDetials?.commentList.reverse();      this.ticketStatusForm.patchValue({
  ...
  submitComment() {
    ...
    this.ticketService.createticketcomment(post).subscribe((res: any) => {
      this.CRUDFunction.apiResponce(res, '') ? (this.getticketdetails(), this.message = '') : '';
    }, (error: any) => {
      this.CRUDFunction.handleHttpError(error);
    });
  }
  ...
  getallticketcategory() {
    this.ticketService.getallticketcategory(true).subscribe((res: any) => {
  ...
  getallprioritiesbyid(categoryId) {
    this.ticketService.getallprioritiesbyid(categoryId).subscribe((res: any) => {
  ...
  getallsubcategorybyid(categoryId) {
    this.ticketService.getallsubcategorybyid(categoryId).subscribe((res: any) => {
  ...
  ticketcategoryidemp(categoryId) {
    ...
    this.ticketService.ticketcategoryidemp(categoryId).subscribe((res: any) => {
  ...
  submitValue(value) {
    ...
    this.ticketService.updateticket(value).subscribe((res: any) => {
      this.CRUDFunction.apiResponce(res, 'Ticket') ? this.CRUDFunction.routingWithOutData('/help-desk/ticket-received') : '';
    }, (error: any) => {
      this.CRUDFunction.handleHttpError(error);
    });
  }
  ...
  upload(files) {
    for (let index = 0; index < files.length; index++) {
      ...
      this.ticketService.uploadticketmultiple(formData).subscribe((res: any) => {
        if (res.success) {
  ...
  reOpenTicket(ticketReOpenForm) {
    this.submitted = true;
    this.CRUDFunction.checkValidation(ticketReOpenForm) ? this.submitReOpenTicekt(ticketReOpenForm.value) : '';
  }
  submitReOpenTicekt(value) {
    value['imageURL'] = this.filesArray[0];
    value['ticketId'] = this.ticketId;
    this.ticketService.updateticketstatus(value).subscribe((res: any) => {
      this.CRUDFunction.apiResponce(res,'Ticket Status') ? this.CRUDFunction.routingWithOutData('/help-desk/ticket-raised') : '';
    }, (error: any) => {
      this.CRUDFunction.handleHttpError(error);
    });
