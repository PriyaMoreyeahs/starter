...
export class TicketCategoryComponent implements OnInit {
  ...
  categoryType = true;  constructor(private CRUDFunction: CRUDFunction, private ticketService: TicketService) { }
  ...
  getallticketcategory(check: boolean) {
    this.ticketService.getallticketcategory(check).subscribe((res: any) => {
  ...
  addUpdateTicketCategory(categoryId?) {
    this.CRUDFunction.routingWithData('/help-desk/add-update-ticket', categoryId)
  }
  //#endregion
}
