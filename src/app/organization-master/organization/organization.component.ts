...
export class OrganizationComponent implements OnInit {
  ...
  constructor(private organizationService: OrganizationService, private CRUDFunction: CRUDFunction, private popupFunctionService: PopupFunctionService, private formBuilder: FormBuilder) {
    this.config = {
      itemsPerPage: 10,
      currentPage: 1,
    }
  ...
  getallactiveorg() {
    this.organizationService.getallactiveorg(this.searchString).subscribe((res: any) => {
  ...
  addUpdateOrganizationPopUp(addUpdateOrganizationComponent, organizationId?) {
    ...
    (this.organizationId = organizationId) ? (this.getorgbyid(organizationId), this.disableRequiredFields()) : '';
    this.popupFunctionService.poupOpenFunction(addUpdateOrganizationComponent, 'lg');
  }
  //#endregion
  ...
  getallcountry() {
    this.organizationService.getallcountry().subscribe((res: any) => {
  ...
  gettimezonelist() {
    this.organizationService.gettimezonelist().subscribe((res: any) => {
  ...
  submitValue(value) {
    let apiName = ''
    this.organizationId ? (apiName = 'editorg', value['orgId'] = this.organizationId) : 'addorg';
    this.organizationService[apiName](value).subscribe((res: any) => {
      this.CRUDFunction.apiResponce(res, 'Organization') ? this.getallactiveorg() : '';
    }, (error) => {
      this.CRUDFunction.handleHttpError(error);
    });
  }
  ...
  getorgbyid(organizationId) {
    this.organizationService.getorgbyid(organizationId).subscribe((res: any) => {
  ...
  disableRequiredFields() {
    let formarray = ['firstName', 'middleName', 'lastName', 'userPhoneNumber', 'personalEmail', 'dateOfBirth', 'gender', 'officeEmail', 'password',];
  ...
  organizationActivityChange(organizationId) {
    this.organizationService.lockorg(organizationId).subscribe((res: any) => {
      this.CRUDFunction.apiResponce(res, 'Organization') ? this.getallactiveorg() : '';
    }, (error) => {
      this.CRUDFunction.handleHttpError(error);
    });
  ...
  exportOrganizationList() {
    ...
    this.CRUDFunction.exportAsExcelFile(table, "Organization List" + new Date())
  }
  //#endregion
}
