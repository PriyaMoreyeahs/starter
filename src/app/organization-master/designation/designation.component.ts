...
export class DesignationComponent implements OnInit {
  ...
  designationonadminview() {
    ...
    this.organizationService.designationonadminview(post).subscribe((res: any) => {
      this.designationInformation = this.CRUDFunction.responceBindingInPagination(res);
  ...
  addUpdateDesignationPopUp(addUpdateDesignationComponent, id?) {
    ...
    this.popupFunctionService.poupOpenFunction(addUpdateDesignationComponent, 'lg');
  }
  //#endregion
  ...
  submitValue(value) {
    ...
    let apiName = ''
    this.designationId ? (apiName = 'updatedesignation', value['designationId'] = this.designationId) : apiName = 'createdesignation';    this.organizationService[apiName](value).subscribe((res: any) => {
      this.CRUDFunction.apiResponce(res, 'Designation') ? this.designationonadminview() : '';
    }, (err: any) => {
  ...
  deleteDesignation(designationId) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.organizationService.deletedesignation(designationId).subscribe((data: any) => {
          if (data.status) {
            Swal.fire(
              "Deleted!",
